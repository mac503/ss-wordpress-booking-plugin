jQuery(document).ready(function () {
  ...
  bookingManager.addNew = function (id) {
    ...
    document.querySelector('#ss_booking_current_action').innerHTML = `
		...
		`;
    if (id) calendar.selectType(document.querySelector('#ss_new_type').selectedOptions[0].value, document.querySelector('#ss_new_type').selectedOptions[0].dataset.massageLength);else calendar.selectType(bookingManager.types[0].name, bookingManager.types[0].length);
  ...
  bookingManager.showList = function (filter, mode) {
    ...
    document.querySelector('#ss_booking_list_list').innerHTML = calendar.bookings.filter(filter).slice().sort((a, b) => {
      if (mode == 'unconfirmed' || mode == 'confirmed') return new Date(a.start) - new Date(b.start);else return new Date(b.start) - new Date(a.start);
    }).map(x => `
    ...
    if (e.target.id == 'ss_booking_list_search_box') {
      ...
      if (e.target.value != '') {
        if (isNaN(parseInt(e.target.value)) == false) {
          [].forEach.call(document.querySelectorAll('.ss_list_item:not([data-id*="' + (parseInt(e.target.value) - 1000) + '"])'), function (nonMatch) {
            nonMatch.style.display = 'none';
          });
        } else [].forEach.call(document.querySelectorAll('.ss_list_item:not([data-name*="' + e.target.value + '"i])'), function (nonMatch) {
          nonMatch.style.display = 'none';
        });
      }
    }
    ...
    if (t.dataset.hasOwnProperty('action')) {
      ...
      switch (t.dataset.action) {
        ...
        case "apply-opening-hours":
          var data = [{
            start: bookingManager.timeMinutes(document.querySelector('#ss_booking_availability_start_0').value),
            end: bookingManager.timeMinutes(document.querySelector('#ss_booking_availability_end_0').value)
          }, {
            start: bookingManager.timeMinutes(document.querySelector('#ss_booking_availability_start_1').value),
            end: bookingManager.timeMinutes(document.querySelector('#ss_booking_availability_end_1').value)
          }, {
            start: bookingManager.timeMinutes(document.querySelector('#ss_booking_availability_start_2').value),
            end: bookingManager.timeMinutes(document.querySelector('#ss_booking_availability_end_2').value)
          }, {
            start: bookingManager.timeMinutes(document.querySelector('#ss_booking_availability_start_3').value),
            end: bookingManager.timeMinutes(document.querySelector('#ss_booking_availability_end_3').value)
          }, {
            start: bookingManager.timeMinutes(document.querySelector('#ss_booking_availability_start_4').value),
            end: bookingManager.timeMinutes(document.querySelector('#ss_booking_availability_end_4').value)
          }];
          bookingManager.setMode('working');
          jQuery.ajax({
            type: "post",
  ...
  jQuery.ajax({
    ...
    success: function (res) {
      window.calendar = new SabaiSaludCalendar({
