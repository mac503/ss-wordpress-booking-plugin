...
SabaiSaludCalendar.prototype.draw = function (div) {
  this.div = div;
  div.classList.add('sabai-salud-calendar'); //draw the calendar (TODO use babel to make sure all browsers can deal with this)

  div.innerHTML = `
    &nbsp;
    <div class='sabai-salud-calendar-header'>
      <span class='sabai-salud-calendar-nav prev'>⇦ Ant.</span><div class='sabai-salud-calendar-date'></div><span class='sabai-salud-calendar-nav next'>Próx. ⇨</span>
    </div>
...
SabaiSaludCalendar.prototype.select = function (date, time, animate = true) {
  ...
  if (this.validateSelection(date, time, this.selectedLength)) {
    ...
    this.selectedTime = time; //get selected date in acceptable form
...
SabaiSaludCalendar.prototype.validateSelection = function (date, time, length) {
  ...
  var availability = JSON.parse(JSON.stringify(this.availability[this.daysBetween(startDate, date)])); //merge bookings with availability

  var excludeBookingId = -1;
  if (this.highlightedBooking) excludeBookingId = this.highlightedBooking.id;
  var dayBookings = this.bookings.filter(x => x.start.substr(0, 10) == this.dateToMySQL(date) && x.deleted != 1 && x.id != excludeBookingId); //find the bookings for that day
  ...
  dayBookings.forEach(function (booking) {
    ...
    if (self.displayPrepTime) bookingSlots += self.prepSlots; //blank out the availability if necessary for the booking
  ...
  if (this.displayPrepTime) slotsRequired += this.prepSlots; //check against modified availability
...
SabaiSaludCalendar.prototype.drawDay = function (date) {
  ...
  this.displayedDate.setHours(0, 0, 0, 0); //set dates of next/prev buttons

  var temp = new Date(this.displayedDate.getTime());
  temp.setDate(temp.getDate() + 1);
  this.div.querySelector('.next').dataset.date = temp;
  temp.setDate(temp.getDate() - 2);
  this.div.querySelector('.prev').dataset.date = temp; //set "allowedness" of next/prev buttons
  ...
  let spanishDisplayDate = this.displayedDate.toLocaleString('es-ES', {
    weekday: "short",
    month: "short",
    day: "numeric"
  }).replace('.', '');
  this.div.querySelector('.sabai-salud-calendar-date').innerHTML = spanishDisplayDate[0].toUpperCase() + spanishDisplayDate.substr(1); //display busy slots as busy
  ...
  while (grains[i]) {
    ...
    if (dayData[i] == 0) grains[i].classList.add('busy');else grains[i].classList.remove('busy');
    i++;
  } //draw any bookings
...
SabaiSaludDateBox.prototype.drawBox = function (date, target, selected) {
  if (selected) selected.setHours(1, 0, 0, 0);else selected = new Date(0);
...
SabaiSaludDateBox.prototype.updateBox = function (date, div, selected) {
  if (selected == null) {
    ...
    if (selectedEl) selected = new Date(parseInt(selectedEl.dataset.date));else selected = new Date(0);
  }

  selected.setHours(1, 0, 0, 0);
  var year = date.getFullYear(); //var month = ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'][date.getMonth()];

  var month = ['enero', 'febrero', 'marzo', 'abril', 'mayo', 'junio', 'julio', 'agosto', 'septiembre', 'octubre', 'noviembre', 'diciembre'][date.getMonth()];
