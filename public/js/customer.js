jQuery(document).ready(function () {
  jQuery.ajax({
    ...
    success: function (res) {
      ...
      document.querySelector('#selected-text').innerHTML = 'Por favor sugiera una fecha  y hora usando el calendario de arriba. <b>Le contactaremos para confirmar disponibilidad.</b>'; //set the selected type (and time)
      ...
      calendar.draw(document.querySelector('#ss_booking_calendar')); //allow the calendar to accept selections, and tell it what else to do when it does
      ...
      calendar.acceptSelection(function (dateString, time, animate) {
        ...
        document.querySelector('#selected-text').innerHTML = `${new Date(dateString).toString().substr(0, 10)} - ${time}. <b>Le contactaremos para confirmar disponibilidad.</b>`;
      }, function () {
        //callback in case selection not allowed
      ...
      calendar.dateBox.drawBox(calendar.displayedDate, calendar.div.querySelector('.sabai-salud-calendar-date'), calendar.displayedDate); //hide the date time entry components - javascript will populate these

      form.querySelector('#dateTimeEntry').style.display = 'none'; //change the selected type when we change it
